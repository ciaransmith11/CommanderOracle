...
export function buildChat(
  ...
  const context = [
    ...
    'CRITICAL — DECK SIZE: the deck MUST total EXACTLY 100 cards: the commander plus EXACTLY 99 other cards. The category counts MUST sum to exactly 99, and each category must actually LIST that many distinct cards. Decide the non-land categories first, then set the Lands count to whatever makes the non-land + land total equal exactly 99 (lands should land ~34–40; if that pushes lands outside a sensible range, adjust another category — never deliver a total other than 99). End with an explicit tally line, e.g. "Total: 38 + 10 + 12 + 12 + 6 + 21 = 99 cards, + Krenko = 100." Do NOT deliver a deck that is not exactly 99 cards plus the commander.',
